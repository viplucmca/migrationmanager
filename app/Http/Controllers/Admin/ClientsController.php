    ...
    public function getnotes(Request $request){
        ...
        foreach($notelist as $list){
            ...
            <div class="note-card-redesign <?php if($list->pin == 1) echo 'pinned'; ?>" data-matterid="<?php echo $list->matter_id; ?>" id="note_id_<?php echo $list->id; ?>" data-type="<?php echo $typeLabel;?>">
                ...
                <div class="note-content-redesign"><?php echo nl2br(htmlspecialchars($descShort, ENT_QUOTES, 'UTF-8'));?>
                    <?php if($showReadMore){ ?>
...
//	echo '<pre>'; print_r($requestData); die;
    ...
    /*public function uploadAgreement(Request $request, $clientId)
    {
        ...
        $command = $libreOfficePath . ' --headless --convert-to pdf --outdir ' . escapeshellarg(dirname($wordFullPath)) . ' ' . escapeshellarg($wordFullPath);
   ...
   /* public function uploadAgreement(Request $request, $clientId)
    {
        ...
        // Pre-process the DOCX to remove potential issues that cause extra pages
        $this->cleanDocxFile($wordFullPath);
        ...
        // Using specific PDF export filter with better quality settings - PRESERVED layout and styling
        $command = $libreOfficePath . ' --headless --convert-to "pdf:writer_pdf_Export:Quality=100:SelectPdfVersion=1:UseTaggedPDF=1:ExportNotes=0:ExportNotesPages=0:UseLosslessCompression=1:EmbedStandardFonts=1:EmbedLatinScriptFonts=1:EmbedAsianScriptFonts=1:EmbedComplexScriptFonts=1:PDFUACompliance=0:ExportFormFields=0:FormsType=0:AllowDuplicateFieldNames=0:ExportBookmarksToPDFDestination=1:ExportBookmarksToPDFNamedDestination=1:ExportPlaceholders=1:ExportHiddenParagraphs=0:SinglePageSheets=0:ExportControl=0:IsSkipEmptyPages=1:IsExportNotesPages=0:IsExportNotes=0:IsExportHiddenParagraphs=0:IsExportBookmarks=1:IsExportBookmarksToPDFDestination=1:IsExportBookmarksToPDFNamedDestination=1:IsExportPlaceholders=1:IsExportFormFields=0:IsExportControl=0" --outdir ' . escapeshellarg(dirname($wordFullPath)) . ' ' . escapeshellarg($wordFullPath);

        // Set environment variables for better quality and consistent page layout
        putenv('LIBREOFFICE_PROFILE_DIR=' . storage_path('app/libreoffice_profile'));
        putenv('SAL_USE_VCLPLUGIN=svp');
        putenv('SAL_DISABLE_OPENGL=1');
        ...
        // Verify PDF was created and check its properties
        if (file_exists($pdfFullPath)) {
            $pdfInfo = shell_exec('pdfinfo "' . $pdfFullPath . '" 2>/dev/null');
            if ($pdfInfo) {
                // Log PDF info for debugging
                \Log::info('PDF conversion info: ' . $pdfInfo);
            }
        }

        // If conversion failed, try alternative method with simpler parameters
        if (!file_exists($pdfFullPath)) {
            $fallbackCommand = $libreOfficePath . ' --headless --convert-to pdf --outdir ' . escapeshellarg(dirname($wordFullPath)) . ' ' . escapeshellarg($wordFullPath);
            exec($fallbackCommand . ' 2>&1', $fallbackOutput, $fallbackResultCode);

            if (!file_exists($pdfFullPath) || $fallbackResultCode !== 0) {
                return response()->json([
                    'status' => false,
                    'message' => 'PDF conversion failed with both methods. Output: ' . implode("\n", $output) . "\nFallback: " . implode("\n", $fallbackOutput)
                ], 500);
            }
        }

        // Additional check: If PDF still has form fields, try a third method with strict static conversion
        if (file_exists($pdfFullPath)) {
            $pdfContent = file_get_contents($pdfFullPath);
            if (strpos($pdfContent, '/AcroForm') !== false || strpos($pdfContent, '/Annots') !== false) {
                \Log::warning('PDF contains form fields, attempting static conversion');

                // Try with different filter that completely disables interactive elements
                $staticCommand = $libreOfficePath . ' --headless --convert-to "pdf:writer_pdf_Export:Quality=100:ExportFormFields=0:ExportControl=0:FormsType=0:UseTaggedPDF=0:ExportNotes=0:ExportNotesPages=0:UseLosslessCompression=1:EmbedStandardFonts=1:EmbedLatinScriptFonts=1:EmbedAsianScriptFonts=1:EmbedComplexScriptFonts=1:PDFUACompliance=0:ExportBookmarksToPDFDestination=0:ExportBookmarksToPDFNamedDestination=0:ExportPlaceholders=0:ExportHiddenParagraphs=0:SinglePageSheets=0" --outdir ' . escapeshellarg(dirname($wordFullPath)) . ' ' . escapeshellarg($wordFullPath);
                exec($staticCommand . ' 2>&1', $staticOutput, $staticResultCode);
            }
        }

        // Post-process PDF to compress but preserve layout
        if (file_exists($pdfFullPath)) {
            $this->compressPdf($pdfFullPath);
        }
     ...
     /**
     * Clean DOCX file to remove elements that cause extra pages while preserving layout
     */
    /*private function cleanDocxFile($filePath)
    {
        try {
            // Extract DOCX content to check for problematic elements
            $zip = new \ZipArchive();
            if ($zip->open($filePath) === TRUE) {
                // Read document.xml to check for hidden elements
                $documentXml = $zip->getFromName('word/document.xml');

                if ($documentXml) {
                    // Remove only truly hidden paragraphs (not those with proper page breaks)
                    $documentXml = preg_replace('/<w:p[^>]*w:val="1"[^>]*>.*?<\/w:p>/s', '', $documentXml);

                    // Preserve page breaks but remove problematic section properties
                    $documentXml = preg_replace('/<w:sectPr[^>]*w:val="1"[^>]*>.*?<\/w:sectPr>/s', '', $documentXml);

                    // Remove form fields and interactive elements
                    $documentXml = preg_replace('/<w:fldSimple[^>]*>.*?<\/w:fldSimple>/s', '', $documentXml);
                    $documentXml = preg_replace('/<w:instrText[^>]*>.*?<\/w:instrText>/s', '', $documentXml);

                    // Preserve font styling and formatting
                    $documentXml = preg_replace('/<w:noProof[^>]*>/', '', $documentXml);

                    // Update the document.xml in the ZIP
                    $zip->addFromString('word/document.xml', $documentXml);
                }

                // Also clean styles to ensure proper font preservation
                $stylesXml = $zip->getFromName('word/styles.xml');
                if ($stylesXml) {
                    // Ensure font styles are preserved
                    $stylesXml = preg_replace('/<w:noProof[^>]*>/', '', $stylesXml);
            ...
            \Log::warning('Failed to clean DOCX file: ' . $e->getMessage());
        }
    }*/

    /**
     * Compress PDF while preserving layout and quality
     */
    /*private function compressPdf($pdfPath)
    {
        try {
            // Check if pdftk is available for PDF manipulation
            $pdftkPath = (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN')
                ? 'pdftk'
                : 'pdftk';

            // Get PDF info to check page count
            $pdfInfo = shell_exec($pdftkPath . ' "' . $pdfPath . '" dump_data 2>/dev/null');

            if ($pdfInfo) {
                // Only compress the PDF without removing pages to preserve layout
                $tempPdfPath = $pdfPath . '.temp';

                // Use pdftk to compress PDF without removing pages
                $command = $pdftkPath . ' "' . $pdfPath . '" cat output "' . $tempPdfPath . '" compress';
                exec($command . ' 2>/dev/null', $output, $resultCode);
                ...
                \Log::info('PDF processed - Page count info: ' . $pdfInfo);
            }
        } catch (\Exception $e) {
            \Log::warning('Failed to compress PDF: ' . $e->getMessage());
        }
    }*/
    ...
    public function uploadAgreement(Request $request, $clientId)
    {
        //1. Validate Word documents (max 50MB)
        $request->validate([
            'agreement_doc' => 'required|mimes:doc,docx|max:51200', // 50MB max
        ]);
        ...
        try {
            ...
            $pythonConverter = app(\App\Services\PythonConverterService::class);

            if ($pythonConverter->isApiAvailable()) {
                // Use Python converter for better quality
                $result = $pythonConverter->convertToPdf($wordFile);
                ...
                if ($result['success']) {
                    ...
                    $s3Path = $clientUniqueId.'/agreement/' . $pdfFileName;
                    \Storage::disk('s3')->put($s3Path, $pdfData);

                    // Save to database
                    $this->saveDocumentRecord($clientId, $originalName, $s3Path, $size, $requestData['clientmatterid']);

                    return response()->json(['status' => true, 'message' => 'Agreement uploaded and converted successfully using Python converter!']);
                }
            }

            // Fallback to improved LibreOffice conversion
            return $this->convertWithLibreOffice($wordFile, $clientId, $originalName, $size, $requestData);
    ...
    /**
     * Convert document using improved LibreOffice settings
     */
    private function convertWithLibreOffice($wordFile, $clientId, $originalName, $size, $requestData)
    {
        $wordPath = $wordFile->store('temp');
        $wordFullPath = storage_path('app/' . $wordPath);
        $pdfFullPath = preg_replace('/\.(docx|doc)$/i', '.pdf', $wordFullPath);
        ...
        $s3Path = $clientUniqueId.'/agreement/' . basename($pdfFullPath);
        \Storage::disk('s3')->put($s3Path, file_get_contents($pdfFullPath));
     ...
     */
    private function saveDocumentRecord($clientId, $originalName, $s3Path, $size, $clientMatterId)
    {
        // Use pathinfo
        $originalInfo = pathinfo($originalName);
        ...
        $fileUrl = Storage::disk('s3')->url($s3Path);
        $doc->myfile = $fileUrl;
        $doc->myfile_key = basename($s3Path);
        $doc->user_id = Auth::user()->id;
        $doc->client_id = $clientId;
        $doc->type = 'client';
        $doc->file_size = $size;
        $doc->doc_type = 'agreement';
        $doc->client_matter_id = $clientMatterId;
        $saved = $doc->save();

        if($saved){
            $subject = 'finalized visa agreement and converted to pdf';
