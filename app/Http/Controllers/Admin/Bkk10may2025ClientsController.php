...
class Bkk10may2025ClientsController extends Controller
{
    ...
    */
	...
	public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'dob' => 'nullable|regex:/^\d{2}\/\d{2}\/\d{4}$/', // Validate DOB in dd/mm/YYYY format
            'age' => 'nullable|string',
            'gender' => 'nullable|in:Male,Female,Other',
            'martial_status' => 'nullable|in:Single,Married,De Facto,Divorced,Widowed',
            'visa_country.0' => 'nullable|string|max:255',
            ...
            'visa_type_hidden.*' => 'nullable|exists:matters,id',
            'visa_expiry_date.*' => 'nullable|regex:/^\d{2}\/\d{2}\/\d{4}$/',
            'visa_description.*' => 'nullable|string|max:255',
            ...
            'expiry_dates.*' => 'nullable|regex:/^\d{2}\/\d{2}\/\d{4}$/', // Added validation for expiry_dates
        ...
        try {
            ...
            // Create the main client record in the admins table
            $client = new Admin();
            $client->first_name = $validated['first_name'];
            $client->last_name = $validated['last_name'] ?? null;
            //$client->dob = $validated['dob'] ? date('Y-m-d', strtotime($validated['dob'])) : null;
            $client->dob = $validated['dob'] ? date('Y-m-d', strtotime(str_replace('/', '-', $validated['dob']))) : null; // Convert dd/mm/YYYY to Y-m-d
            ...
            // Save visa details
            if (!empty($validated['visa_type_hidden']) && $validated['visa_country'][0] !== 'Australia') {
                foreach ($validated['visa_type_hidden'] as $index => $visa_type) {
                    ClientVisaCountry::create([
                        'client_id' => $client->id,
                        'admin_id' => Auth::user()->id,
                        'visa_type' => $visa_type,
                        'visa_expiry_date' => !empty($validated['visa_expiry_date'][$index])
                            ? date('Y-m-d', strtotime(str_replace('/', '-', $validated['visa_expiry_date'][$index])))
                            : null,
                        'visa_description' => $validated['visa_description'][$index] ?? null,
                    ]);
                }
            }
            ...
            if (!empty($validated['address'])) {
                ...
                if ($count > 0) {
                    // Get the last address (which will be saved as the most recent)
                    $lastIndex = $count - 1;
                    ...
                    //dd($validated['is_current_address']);
                    $isCurrentAddress = isset($request->is_current_address) && $request->is_current_address === '1';
                    // Debug: Log the checkbox value
			        \Log::debug('isCurrentAddress value: ' . ($isCurrentAddress ? 'true' : 'false'));
                    ...
                    // Process each address in reverse order (last address first)
                    foreach ($reversedKeys as $index => $key) {
                        ...
                        if (!empty($addr) || !empty($zip)) {
                           // Set is_current to 1 for the last address saved (first in the form array) if the checkbox is checked, otherwise 0
                            ...
                            // Debug: Log the current index, key, and isCurrent value
                            \Log::debug("Address index: {$index}, key: {$key}, isCurrent: {$isCurrent}");
            ...
            // Save spouse details (if applicable)
            if ($validated['martial_status'] === 'Married') {
            ...
            if( isset($validated['type']) && $validated['type'] == 'lead') {
            ...
            if( isset($validated['type']) && $validated['type'] == 'lead') {
                ...
                return redirect()->back()->withErrors(['error' => 'Failed to create lead. Please try again.']);
            } else {
                // Log the error for debugging
                \Log::error('Client creation failed: ' . $e->getMessage());
                // Redirect back with error message
                return redirect()->back()->withErrors(['error' => 'Failed to create client. Please try again.']);
            }
        }
    }
    ...
    public function edit(Request $request, $id = NULL)
    {
        ...
        if ($request->isMethod('post')) {
            ...
            // Add validation for visa fields only if the country is not Australia
            if (isset($requestData['visa_country']) && is_array($requestData['visa_country']) && $requestData['visa_country'][0] !== 'Australia') {
                $validationRules['visa_type_hidden.*'] = 'required|string|max:255';
                $validationRules['visa_expiry_date.*'] = 'nullable|date_format:d/m/Y|after_or_equal:1000-01-01'; // Ensure date is valid for MySQL
            ...
            // Visa Country Handling (unchanged)
            if (
                ...
                if (isset($requestData['visa_country']) && $requestData['visa_country'][0] === 'Australia') {
                    // If the country is Australia, save minimal visa details
                    ClientVisaCountry::create([
                        'admin_id' => Auth::user()->id,
                        'client_id' => $obj->id,
                        'visa_country' => $requestData['visa_country'][0],
                        'visa_type' => "",
                        'visa_expiry_date' => null, // Explicitly set to null for Australia
                        'visa_description' => ""
                    ]);
                    ...
                    // If the country is not Australia, save visa details if provided
                    if (isset($requestData['visa_type_hidden'])) {
                        foreach ($requestData['visa_type_hidden'] as $key => $visaType) {
                            ...
                            // Convert visa_expiry_date from dd/mm/yyyy to Y-m-d for database storage
                            $formatted_expiry_date = null;
                            if (!empty($visa_expiry_date)) {
                                try {
                                    $date = \Carbon\Carbon::createFromFormat('d/m/Y', $visa_expiry_date);
                                    $formatted_expiry_date = $date->format('Y-m-d');
                                } catch (\Exception $e) {
                                    // If date conversion fails, return an error
                                    return redirect()->back()->withErrors(['visa_expiry_date.' . $key => 'Invalid Visa Expiry Date format: ' . $visa_expiry_date])->withInput();
                                }
                            }
                            ...
                            if (!empty($visaType) && !empty($visa_country)) {
                                ClientVisaCountry::create([
                                    ...
                                    'visa_type' => $visaType,
                                    'visa_expiry_date' => $formatted_expiry_date, // Use the formatted date
                        ...
                        if ($count_visa > 0) {
                            ...
                            // Convert the last visa expiry date for the Admin model
                            ...
                            if (!empty($lastVisaExpiryDate)) {
                                try {
                                    ...
                                    // This should already be caught by the validation, but just in case
                                    $lastFormattedExpiryDate = null;
                                }
                            }
            ...
            if ( isset($requestData['nomi_occupation']) && is_array($requestData['nomi_occupation']))
            {
                ...
                \Log::info('Occupation Data Received:', [
                    ...
                    'visa_subclass' => $requestData['visa_subclass'] ?? [],
            ...
            // Update total points, type, and source (unchanged)
            $obj->total_points = $requestData['total_points'] ?? null;
    ...
    public function addedudocchecklist(Request $request){ //dd($request->all());
                                ...
                                <div class="grid_content">
                                    ...
                                    <div class="dropdown d-inline dropdown_ellipsis_icon">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            <a data-id="<?php echo $fetch->id; ?>" class="dropdown-item deletenote" data-doccategory="<?php echo $request->folder_name;?>" data-href="deletedocs" href="javascript:;" >Delete</a>
                                            <a data-id="<?php echo $fetch->id; ?>" class="dropdown-item verifydoc" data-doctype="personal" data-doccategory="<?php echo $request->folder_name;?>" data-href="verifydoc" href="javascript:;">Verify</a>
                            ...
                            { ?>
                                <div class="dropdown d-inline">
                                    ...
                                    <div class="dropdown-menu">
                                        ...
                                        <?php } ?>
                                        <a download class="dropdown-item" href="<?php echo $fetch->myfile; ?>">Download</a>
                                        <a data-id="<?php echo $fetch->id; ?>" class="dropdown-item deletenote" data-doccategory="<?php echo $request->doccategory;?>" data-href="deletedocs" href="javascript:;" >Delete</a>
                                        <a data-id="<?php echo $fetch->id; ?>" class="dropdown-item verifydoc" data-doctype="personal" data-doccategory="<?php echo $request->doccategory;?>" data-href="verifydoc" href="javascript:;">Verify</a>
    ...
    public function addvisadocchecklist(Request $request){ //dd($request->all());
        ...
        //$folder_name = isset($request->folder_name)? $request->folder_name : '';
        $doctype = isset($request->doctype)? $request->doctype : '';
        if ($request->has('visa_checklist'))
        {
            ...
            if (is_array($checklistArray))
            {
                foreach ($checklistArray as $item)
                {
                    ...
                    $obj->type = $request->type;
                    $obj->doc_type = $doctype;
                    $obj->client_matter_id = $request->hidden_client_matter_id;
                    $obj->checklist = $item;
                    $saved = $obj->save();
                }  //end foreach
                ...
                if($saved)
                {
                    ...
                    //Update date in client matter table
                    if( isset($request->hidden_client_matter_id) && $request->hidden_client_matter_id != ""){
                        $obj1 = \App\ClientMatter::find($request->hidden_client_matter_id);
                        $obj1->updated_at = date('Y-m-d H:i:s');
                        $obj1->save();
                    }
                    $response['status'] 	= 	true;
                    $response['message']	=	'You’ve added uploaded your visa checklist';
                    ...
                    {
                        $admin = \App\Admin::where('id', $fetch->user_id)->first();
                        if($request->hidden_client_matter_id != $fetch->client_matter_id){
                        ...
                        ?>
                        <tr class="drow" data-matterid="<?php echo $fetch->client_matter_id;?>" id="id_<?php echo $fetch->id; ?>" <?php echo $showCls;?>>
                                ...
                                <div class="grid_content">
                                    ...
                                    <div class="dropdown d-inline dropdown_ellipsis_icon">
                                        ...
                                        <div class="dropdown-menu">
                                            ...
                                            <a data-id="<?php echo $fetch->id; ?>" class="dropdown-item verifydoc" data-doctype="visa" data-href="verifydoc" href="javascript:;">Verify</a>
    ...
    public function uploadvisadocument(Request $request){  //dd($request->all());
        ...
        if ($request->hasfile('document_upload')) {
            ...
            if($saved){
				...
				$response['status'] 	= 	true;
				$response['message']	=	'You’ve successfully uploaded your visa document';
...
//	echo '<pre>'; print_r($requestData); die;
    ...
    /*public function saveaccountreport(Request $request, $id = NULL)
	{
        ...
        if(isset($requestData['trans_date'])){
            ...
            $finalArr = array();
            $running_balance = $requestData['client_ledger_balance_amount'];
            ...
            for($i=0; $i<count($requestData['trans_date']); $i++)
            {
                ...
                //Generate Logic For unique transaction number based on client_fund_ledger_type
                $clientFundLedgerType = $requestData['client_fund_ledger_type'][$i];
                $finalArr[$i]['client_fund_ledger_type'] = $clientFundLedgerType;

                //Start Logic For trans no

                    // Set prefix based on ledger type
                    ...
                    // Get the last transaction number with this type
                    ...
                    $trans_no = $prefix . '-' . $formattedNumber;
                    $finalArr[$i]['trans_no'] = $trans_no;
                //End Logic For trans no

                $finalArr[$i]['description'] = $requestData['description'][$i];

                //Start Logic For balance amount
                    // Ensure these are casted to float for safe math
                    $deposit = floatval($requestData['deposit_amount'][$i] ?? 0);
                    $withdraw = floatval($requestData['withdraw_amount'][$i] ?? 0);

                    $finalArr[$i]['deposit_amount'] = $deposit;
                    $finalArr[$i]['withdraw_amount'] = $withdraw;

                    // Now calculate balance properly
                    //dd($deposit.'==='.$withdraw.'==='.$running_balance);
                    $running_balance += $deposit - $withdraw;
                    $finalArr[$i]['balance_amount'] = $running_balance;
                //End Logic For balance amount

                if( isset($requestData['invoice_no'][$i]) && $requestData['invoice_no'][$i] != ""){
                ...
                $saved	= DB::table('account_client_receipts')->insert([
                    ...
                    'trans_no' => $trans_no,
                    'client_fund_ledger_type' => $requestData['client_fund_ledger_type'][$i],
                ...
                if( $saved && $doc_saved != ""){
                    $subject = 'added client funds ledger with its document. Reference no- '.$trans_no;
                } else {
                    $subject = 'added client funds ledger.Reference no- '.$trans_no;
                }
                if($request->type == 'client'){
                    $objs = new ActivitiesLog;
                    $objs->client_id = $requestData['client_id'];
            ...
            if (isset($requestData['invoice_no']) && $requestData['invoice_no'] != "")
            {
                ...
                $uniqueInvoiceNos = array_unique(array_filter($requestData['invoice_no']));
                //dd($uniqueInvoiceNos);
                // Loop through each unique invoice and update its status
                foreach ($uniqueInvoiceNos as $invoiceNo) {
                    $clientId = $requestData['client_id'];
                    // Get invoice total amount (assuming it's the sum of all withdraw_amounts from request for this invoice)
                    $invoiceTotal = 0;
                    for ($ii = 0; $ii < count($requestData['invoice_no']); $ii++) {
                        if ($requestData['invoice_no'][$ii] == $invoiceNo) {
                            $invoiceTotal += $requestData['withdraw_amount'][$ii] ?? 0;
                        }
                    }

                    // Get total withdrawn from DB for this invoice
                    $totalWithdrawn = DB::table('account_client_receipts')
                    ...
                    if ($invoiceTotal  < $totalWithdrawn ) {
                        $status = 2; // Partial
                        $partial_paid_amount = $invoiceTotal;
                        $balance_amount1 = $totalWithdrawn - $invoiceTotal;
                    } else {
                        $status = 1; // Paid
                    ...
                    // Update all rows with this invoice
                    ...
                    $response['invoice_status'] = $status;
                    $response['invoice_balance'] = $balance_amount1;
                    $response['invoice_no'] = $invoiceNo;

                     // Get total withdrawn from DB for this invoice
                    $outstanding_balance = DB::table('account_client_receipts')
                    ->where('client_id', $clientId)
                    ->where('receipt_type', 3)
                    ->where('invoice_no', $invoiceNo)
                    ->sum('balance_amount');
            ...
            try {
                ...
                foreach ($requestData['trans_date'] as $index => $transDate) {
                    $entryData = [
                        ...
                        'withdraw_amount' => $requestData['withdraw_amount'][$index],
                        'invoice_no' => $requestData['invoice_no'],
                        'save_type' => $requestData['save_type'],
                        'updated_at' => $currentTimestamp, // Add updated_at timestamp
                    ];

                    $totalWithdrawAmount += (float)$requestData['withdraw_amount'][$index];
	...
	{
        $requestData 		= 	$request->all();
        $record_get = DB::table('account_client_receipts')->select('invoice_no')->where('client_id',$requestData['client_id'])->where('receipt_type',3)->where('save_type','final')->distinct()->get();
        //dd($record_get);
        if(!empty($record_get)) {
    ...
    public function genInvoice(Request $request, $id){
        ...
        $total_amount = DB::table('account_all_invoice_receipts')->where('receipt_type',3)->where('receipt_id',$id)->sum('withdraw_amount');

        $total_amount_with_gst_yes = DB::table('account_all_invoice_receipts')->where('receipt_type',3)->where('gst_included','Yes')->where('receipt_id',$id)->sum('withdraw_amount');

        $total_gst_amount = ($total_amount_with_gst_yes*10)/100;
        $total_invoice_amount =  $total_amount - $total_gst_amount;
        ...
        $receipt_info = DB::table('account_client_receipts')->select('partial_paid_amount','balance_amount')->where('receipt_type',3)->where('receipt_id',$id)->first();
        ...
        //Get client matter
        if( !empty($record_get) && $record_get[0]->client_id != '') {
            $client_matter_no = '';
            ...
            $matter_info = DB::table('client_matters')->select('client_unique_matter_no')->where('client_id',$record_get[0]->client_id)->first();
            if($matter_info){
		...
		])->loadView('emails.geninvoice',compact(['record_get','record_get_Professional_Fee_cnt','record_get_Department_Charges_cnt','record_get_Surcharge_cnt','record_get_Disbursements_cnt','record_get_Other_Cost_cnt','record_get_Professional_Fee','record_get_Department_Charges','record_get_Surcharge','record_get_Disbursements','record_get_Other_Cost','total_invoice_amount','total_gst_amount','total_amount','clientname','receipt_info','invoice_payment_method','client_matter_no']));
		//
    ...
    public function invoicelist(Request $request)
	{
        ...
        $lists = $query->sortable(['id' => 'desc'])->paginate(20);
		return view('Admin.clients.invoicelist', compact(['lists', 'totalData']));
    }
    ...
    public function officereceiptlist(Request $request)
	{
        ...
        $totalData 	= $query->count();
        $lists = $query->sortable(['id' => 'desc'])->paginate(20);
		return view('Admin.clients.officereceiptlist', compact(['lists', 'totalData']));
    }
        ...
        if( isset($request->clickedReceiptIds) && !empty($request->clickedReceiptIds) ){
            ...
            if ($affectedRows > 0) {
                ...
                foreach($request->clickedReceiptIds as $ReceiptVal){
                    $receipt_info = AccountClientReceipt::select('user_id','client_id')->where('id', $ReceiptVal)->first();
    ...
    public function addRelation(Request $request)
    {
        $request->validate([
            ...
            'relation_type' => 'required|in:Father,Mother,Wife,Son,Daughter,Sister,Uncle,Aunt,Brother',
        ]);

        DB::beginTransaction();
        try {
            $clientId = $request->client_id;
            $relatedUserId = $request->related_user_id;
            $relationType = $request->relation_type;

            // Check for duplicate relation
            $existingRelation = Relationship::where('user_id', $clientId)
                ->where('related_user_id', $relatedUserId)
                ->where('relation_type', $relationType)
                ->exists();

            if ($existingRelation) {
                return response()->json(['message' => 'This relation already exists'], 422);
            }

            // Determine reciprocal relation
            $reciprocalMap = [
                'Father' => 'Son',
                'Mother' => 'Daughter',
                'Wife' => 'Husband',
            ...
            $reciprocalType = $reciprocalMap[$relationType] ?? 'Relative';
            ...
            DB::rollBack();
            return response()->json(['message' => 'Error adding relation'], 500);
        }
    }

    public function getRelationships($clientId)
    {
        $relationships = Relationship::with('relatedUser')
